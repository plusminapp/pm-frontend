...
![[Betalingen](#betaalgroep) [koppelen](#koppelen) aan [potjes](#potje)](images/h4-koppelen.png)
...
Met de knop *Gebruiken* ga je verder naar de volgende stap. Gebruik deze knop pas als je alle [betalingen](#betaalgroep) hebt [gekoppeld](#koppelen). In hoofdstuk 6 wordt uitgelegd wat [Gebruiken](#6-stap-4-gebruiken) is.
...
Hiermee houd je grip op grote aantallen [betalingen](#betaalgroep). Je hoeft niet alles tegelijk te doen: zoek de [betaalgroepen](#betaalgroep) waar nog wat mee moet gebeuren. Met *Filteren* kun je precies die groepen overhouden die je wilt [koppelen](#koppelen).
...
Onder de tabbladen zit, helemaal links, het *Bewerking*-veld. Je kunt kiezen uit *Zoeken/samenvoegen* (is de standaardkeuze) en *[Koppelen zonder regel](#koppelen-zonder-regel)*.
...
#### Koppelen zonder regel

Voor sommige [betalingen](#betaalgroep) wil je helemaal geen regel maken om te [koppelen](#koppelen). In de paragraaf [Koppelingsregels](#koppelingsregels) staat meer informatie over die regels. Als je op vakantie in een barretje wat hebt gedronken, hoeft dat later nooit meer [gekoppeld](#koppelen) te worden. Eén keer en nooit weer. Je hebt daar wel geld uitgegeven en wilt het ook aan een [potje](#potje) [koppelen](#koppelen), maar [zonder regel](#koppelen-zonder-regel). 

Kies eerst in het *Bewerking*-veld voor *[Koppelen zonder regel](#koppelen-zonder-regel)*. Als je dan een aantal [betaalgroepen](#betaalgroep) kiest (door de checkboxes aan te klikken), wordt de knop 'Koppelen zonder regel' actief. Als je daar op klikt, kun je kiezen aan welk [potje](#potje) je die [betalingen](#betaalgroep) wilt [koppelen](#koppelen). Je kunt alleen kiezen voor [Leefgeld](#leefgeld)-[potjes](#potje). Voor alle andere [categorieën](#categorie) is een regel wel handig.

Het kan zijn dat je een tussenscherm met waarschuwingen krijgt. Lees de waarschuwing en kies om wel of niet door te gaan.

[Koppelen zonder regel](#koppelen-zonder-regel) is erg handig als je er bijna bent: je hebt bijna alles [gekoppeld](#koppelen), maar er blijven losse [betalingen](#betaalgroep) over. Je kiest het tabblad [Onbekend](#onbekend), selecteert ze allemaal en [koppelt](#koppelen) ze allemaal aan één [potje](#potje). 
...
### 4.6. De betalingen koppelen aan een potje

In de [betalingen](#betaalgroep)-lijst doe je het echte werk. Hier gaan we de [betaalgroepen](#betaalgroep) [koppelen](#koppelen) aan een [potje](#potje). Het is erg simpel.

Zorg eerst dat alle [betalingen](#betaalgroep) die je wilt [koppelen](#koppelen) in één groep zitten. Hoe je dat doet, staat in [paragraaf 4.5](#45-filteren-sorteren-en-bewerken-h4-5). Dat is vooral voor het gemak en niet echt noodzakelijk. Hoe meer [betalingen](#betaalgroep) je in 1x kunt [koppelen](#koppelen), hoe sneller je klaar bent.

Klik op het potlood-icoontje, helemaal rechts, bij een groep om te [koppelen](#koppelen). Het 'Potjes [koppelen](#koppelen)'-schermpje verschijnt.
...
Om met dat 'bijna' te beginnen: als je uit bent geweest, jij hebt betaald, en je hebt Tikkies gestuurd, dan rekenen we die [betalingen](#betaalgroep) niet tot [inkomsten](#inkomsten). Die [koppel](#koppelen) je aan het [potje](#potje) waar je de [betaling](#betaalgroep) aan hebt [gekoppeld](#koppelen). Wat dan overblijft, is precies wat je zelf hebt uitgegeven. 
...
Als het niet nodig is dat er een regel wordt aangemaakt voor deze [tegenpartij](#tegenpartij), kies je voor [Leefgeld](#leefgeld) [zonder regel](#koppelen-zonder-regel). De [betaling](#betaalgroep) wordt wel aan een [potje](#potje) [gekoppeld](#koppelen), maar [zonder regel](#koppelen-zonder-regel). Een volgende keer wordt de [betaling](#betaalgroep) niet automatisch [gekoppeld](#koppelen).
...
[Negeren](#negeren) is een buitenbeentje in de [categorieën](#categorie). Stel je hebt dit jaar een flinke belastingteruggave gehad. Mazzeltje. Je bent nu aan het budgetteren voor komende jaren. Je verwacht de komende jaren geen andere belastingteruggaven, dus wilt je hem niet meenemen in je [budgetten](#budget). Dan [koppel](#koppelen) je de [betaling](#betaalgroep) aan [Negeren](#negeren). Dan telt hij niet mee.

Andersom kan natuurlijk ook: je hebt jezelf dit jaar getracteerd op een nieuwe gitaar, omdat je die belastingteruggave had. Dat soort uitspattingen zit er voorlopig niet in. Dan [koppel](#koppelen) je de gitaar ook aan de [categorie](#categorie) [Negeren](#negeren). 
...
Naast de Groepsnaam en de [Categorie](#categorie) moet de [betaalgroep](#betaalgroep) ook nog aan een [potje](#potje) worden [gekoppeld](#koppelen). Dat doe je in het onderste veld.

Als je een [categorie](#categorie) hebt gekozen en je klikt in het [Potje](#potje)-veld, zie je alle [potjes](#potje) die nu al aan die [categorie](#categorie) zijn [gekoppeld](#koppelen). 
...
#### Koppelen zonder regel
Een optie om een [betaling](#betaalgroep) aan een [potje](#potje) te [koppelen](#koppelen) zonder dat er een automatische regel wordt aangemaakt voor toekomstige [betalingen](#betaalgroep).

#### Koppelingsregels
Regels die de app automatisch aanmaakt om toekomstige [betalingen](#betaalgroep) van dezelfde [tegenpartij](#tegenpartij) automatisch correct te [koppelen](#koppelen).
...
#### Stepper
De navigatiebalk bovenaan elke pagina met vier stappen: [Welkom](#2-stap-1-welkom), [Uploaden](#3-stap-2-bestanden-uploaden), [Koppelen](#koppelen) en [Gebruiken](#6-stap-4-gebruiken).
