...
export default function Sparen() {
  ...
  useEffect(() => {
        stand && setSparenSaldi(stand.resultaatOpDatum.
          filter((r: SaldoDTO) => r.rekeningGroepSoort === RekeningGroepSoort.spaarrekening)
        );
  }, [stand]);
    ...
    <>
      {sparenSaldi.length === 0 &&
        <>
          ...
          <Typography variant='body2'>{actieveHulpvrager?.bijnaam} heeft op {gekozenPeriode?.periodeStartDatum} geen spaartegoed ingericht.</Typography>
          <PeriodeSelect />
        </>
