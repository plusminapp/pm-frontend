...
function resolveDocAssetPath(src?: string): string | undefined {
  ...
  return `/docs/bankoverzicht/${src.replace(/^\.\//, '')}`
}
...
export function BankOverzichtHelp() {
  ...
  useEffect(() => {
    ...
    fetch('/docs/bankoverzicht/BankOverzicht.md')
      .then((response) => {
        if (!response.ok) {
    ...
    return (
      <div className="py-12 px-[10%]">
        ...
        <Button
          ...
          onClick={() => navigate('/bankoverzicht')}
          sx={{ mt: 2 }}
        >
          Terug naar het bankoverzicht
        </Button>
      </div>
    )
  }
      ...
      {tocItems.length > 0 && (
        <nav className="mb-8 rounded-lg border border-gray-200 bg-gray-50" aria-label="Inhoudsopgave">
          <button
            ...
            aria-controls="bankoverzicht-help-toc"
            onClick={() => setIsTocOpen((open) => !open)}
          >
            <h2 className="text-lg font-semibold text-gray-900">Inhoud</h2>
            <span className="text-sm text-gray-600">{isTocOpen ? 'Verbergen' : 'Toon'}</span>
          </button>
          {isTocOpen && (
            <div id="bankoverzicht-help-toc" className="border-t border-gray-200 px-4 pb-4 pt-3">
