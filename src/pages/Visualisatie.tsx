...
export default function Visualisatie() {
  ...
  return (null);
    ...
    //       </Grid>
    //     </Grid>
    //     {formFields.rekeningGroepSoort !== 'aflossing' && formFields.budgetten.map((budget, index) =>
    ...
    //           />
    //           <Typography variant='body2' sx={{ fontSize: '0.875rem', ml: 1 }}>
    //             is {budget.rekeningGroepSoort === 'inkomsten' ? 'ontvangen' : 'betaald'}.
    //           </Typography>
    //         </Grid>
    //         <Grid size={1} >
    ...
    //       </Grid>
    //     )}
        {/* {formFields.rekeningGroepSoort === 'aflossing' && formFields.aflossingen.map((aflossing, index) =>
          <Grid key={aflossing.RekeningGroep.naam + index} container spacing={2} alignItems="center" columns={2} justifyContent={'start'}>
            <Grid size={1} display={'flex'} flexDirection={'row'} alignItems={'center'}>
              <Typography key={index} variant='body2' sx={{ fontSize: '0.875rem', ml: 1 }}>
                {aflossing.RekeningGroep.naam}: {formatAmount(aflossing.aflossingsBedrag.toString())}, betaaldag {aflossing.betaalDag && dagInPeriode(aflossing.betaalDag, periode).format('D MMMM')}, waarvan er
              </Typography>
              <TextField
                label=""
                sx={{ fontSize: '0.875rem', ml: 1, width: '95px', textAlign: 'right' }}
      ...
      {/* {periode &&
        <>
          {formFields.budgetType === BudgetType.continu &&
            <BudgetContinuGrafiek
              ...
              RekeningGroep={{
                ...
                rekeningGroepSoort: rekeningGroepSoort.uitgaven,
          ...
          {formFields.budgetType === BudgetType.vast && formFields.rekeningGroepSoort === 'uitgaven' &&
            <BudgetVastGrafiek
              visualisatie={'all'}
              periode={periode}
              peilDatum={peilDatum}
          ...
          {formFields.budgetType === BudgetType.vast && formFields.rekeningGroepSoort === 'inkomsten' &&
            <BudgetInkomstenGrafiek
              ...
              RekeningGroep={{
                ...rekeningTemplate,
                naam: 'Inkomsten',
                rekeningGroepSoort: rekeningGroepSoort.inkomsten,
                budgetType: BudgetType.vast,
              }}
              budgetten={formFields.budgetten} />}
          {formFields.rekeningGroepSoort === 'aflossing' &&
            <AflossingGrafiek
              visualisatie={'all'}
              periode={periode}
              peilDatum={peilDatum}
    ...
    //       <InputLabel id="RekeningGroep-select-label" sx={{ fontSize: '0.8rem', ml: 1 }}>RekeningGroep</InputLabel>
    //       <Select
    //         labelId="RekeningGroep-select-label"
    //         variant="standard"
    //         id="RekeningGroep-select"
    ...
    //       >
    //         {['Samenvatting', 'Inkomsten', 'Boodschappen', 'Vaste lasten', 'Aflossing'].map((RekeningGroep) => (
    //           <MenuItem key={RekeningGroep} value={RekeningGroep}>
    //             {RekeningGroep}
    //           </MenuItem>
    //         ))}
    //       </Select>
