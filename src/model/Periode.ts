...
export const isPeriodeOpen = (gekozenPeriode: Periode) =>
...
export const eersteOpenPeriode = (periodes: Periode[]): Periode | undefined => {
  const openPeriodes = periodes
    ...
    .sort((a, b) =>
      dayjs(a.periodeStartDatum).diff(dayjs(b.periodeStartDatum)),
    );

  return openPeriodes.length > 0 ? openPeriodes[0] : undefined;
};

export const laatsteGeslotenPeriode = (
  periodes: Periode[],
): Periode | undefined => {
  const geslotenPeriodes = periodes
    .filter(
      (p) => p.periodeStatus === 'GESLOTEN' || p.periodeStatus === 'OPGERUIMD',
    )
    .sort((a, b) =>
