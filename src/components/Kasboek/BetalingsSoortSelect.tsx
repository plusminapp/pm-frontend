...
const BetalingsSoortSelect = (props: BetalingsSoortSelectProps) => {
  ...
  const [selectedCategorie, setSelectedCategorie] = useState<
    string | undefined
  >(betalingsSoort2Categorie(props.betaling?.betalingsSoort));
  const [selectedBetalingsSoort, setSelectedBetalingsSoort] = useState<
    BetalingsSoort | undefined
  >(props.betaling?.betalingsSoort);
  const [selectedRekeningGroep, setSelectedRekeningGroep] = useState<
  ...
  useEffect(() => {
    if (props.betaling) {
      const rekeningGroepen = rekeningGroepPerBetalingsSoort.find(
        (rgpb) => rgpb.betalingsSoort === props.betaling?.betalingsSoort,
      )?.rekeningGroepen;
      const rekening =
        props.betaling?.betalingsSoort &&
        bestemmingBetalingsSoorten.includes(props.betaling.betalingsSoort)
          ? rekeningGroepen
              ?.find((rg) =>
                rg.rekeningen.some((r) => r.naam === props.betaling?.bron),
              )
              ?.rekeningen.find((r) => r.naam === props.betaling?.bron)
          : rekeningGroepen
              ?.find((rg) =>
                rg.rekeningen.some(
                  (r) => r.naam === props.betaling?.bestemming,
                ),
              )
              ?.rekeningen.find((r) => r.naam === props.betaling?.bestemming);
      const betaalMethode =
        props.betaling?.betalingsSoort &&
        bestemmingBetalingsSoorten.includes(props.betaling.betalingsSoort)
          ? rekening?.betaalMethoden?.find(
              (bm) => bm.naam === props.betaling?.bestemming,
            )
          : rekening?.betaalMethoden?.find(
              (bm) => bm.naam === props.betaling?.bron,
            );
      const rekeningGroep = rekeningGroepen?.find(
        (rg) => rekening?.rekeningGroepNaam === rg.naam,
      );
  ...
  const categorie2DefaultBetalingsSoort = (
    categorie: string,
  ): BetalingsSoort | undefined => {
    ...
    if (
      categorie === 'INTERN' &&
      rekeningGroepPerBetalingsSoort.filter(
        (rgpb) => rgpb.betalingsSoort === BetalingsSoort.incasso_creditcard,
      ).length > 0
    )
      return BetalingsSoort.incasso_creditcard;
    if (
      categorie === 'INTERN' &&
      rekeningGroepPerBetalingsSoort.filter(
        (rgpb) => rgpb.betalingsSoort === BetalingsSoort.opnemen,
      ).length > 0
    )
      return BetalingsSoort.opnemen;
    if (
      categorie === 'INTERN' &&
      rekeningGroepPerBetalingsSoort.filter(
        (rgpb) => rgpb.betalingsSoort === BetalingsSoort.opnemen_contant,
      ).length > 0
    )
      return BetalingsSoort.opnemen_contant;
    return undefined;
  };
  ...
  const handleBetalingsSoortClick = (
    betalingsSoort: BetalingsSoort | undefined,
  ) => {
    if (
      betalingsSoort === undefined ||
      selectedBetalingsSoort?.toString() === betalingsSoort.toString()
    ) {
      setSelectedCategorie(undefined);
      ...
      const newBron = bestemmingBetalingsSoorten.includes(newBetalingsSoort)
        ? newRekening.naam
        : newBetaalMethode?.naam;
      const newBestemming = bestemmingBetalingsSoorten.includes(
        newBetalingsSoort,
      )
        ? newBetaalMethode?.naam
        : newRekening.naam;
  ...
  const handleRekeningGroepClick = (rekeningGroepNaam: string) => {
    const newBetalingsSoort = rekeningGroepPerBetalingsSoort.find((rgpb) =>
      rgpb.rekeningGroepen.some((rg) => rg.naam === rekeningGroepNaam),
    )?.betalingsSoort;
    const newRekeningGroep = rekeningGroepPerBetalingsSoort
      .find((rgpb) => rgpb.betalingsSoort === newBetalingsSoort)
      ?.rekeningGroepen.find((rg) => rg.naam === rekeningGroepNaam);
    const newRekening = newRekeningGroep?.rekeningen[0];
    const newBetaalMethode = newRekening?.betaalMethoden
      ? newRekening.betaalMethoden[0]
      : undefined;
  ...
  const handleRekeningClick = (rekeningNaam: string) => {
    const newRekening = selectedRekeningGroep?.rekeningen.find(
      (r) => r.naam === rekeningNaam,
    );
    const newBetaalMethode = newRekening?.betaalMethoden
      ? newRekening.betaalMethoden[0]
      : undefined;
  ...
  const handleBetaalMethodeClick = (betaalMethodeNaam: string) => {
    const newBetaalMethode = selectedRekening?.betaalMethoden?.find(
      (bm) => bm.naam === betaalMethodeNaam,
    );

    const newBron =
      selectedBetalingsSoort &&
      bestemmingBetalingsSoorten.includes(selectedBetalingsSoort)
        ? selectedRekening?.naam
        : newBetaalMethode?.naam;
    const newBestemming =
      selectedBetalingsSoort &&
      bestemmingBetalingsSoorten.includes(selectedBetalingsSoort)
          ...
          {rekeningGroepPerBetalingsSoort
            ...
            .map((rekeningGroep, index) => (
              <Button
                ...
                sx={{ mb: '13px' }}
                variant={
                  selectedRekeningGroep?.naam === rekeningGroep.naam
                    ? 'contained'
                    : 'outlined'
                }
                  ...
                  rg.rekeningen
                    ...
                    .map(
                      (rekening) =>
                        ...
                        rekening.betaalMethoden.length > 1 && (
                          <Fragment key={rekening.naam + '-betaalmethoden'}>
                            <Typography
                              key={rekening.naam + '-text'}
                              textAlign={'center'}
                              fontSize={'12px'}
                              color="grey"
                              marginTop={'12px'}
                            ...
                            >
                              {rekening.betaalMethoden.map((betaalMethode) => (
                                <Button
                                  ...
                                  onClick={() =>
                                    handleBetaalMethodeClick(betaalMethode.naam)
                                  }
                                >
                                  {betaalMethode.naam}
                                </Button>
  ...
  return (
    <div>
      <Grid container spacing={5} justifyContent="center">
        <Box>
          <Button
            color="success"
            variant={
              selectedCategorie === 'INKOMSTEN' ? 'contained' : 'outlined'
            }
            onClick={() => handleCategorieClick('INKOMSTEN')}
          >
            <InkomstenIcon
              color={selectedCategorie === 'INKOMSTEN' ? 'white' : 'success'}
            />
          </Button>
          <Typography
            fontSize={11}
        ...
        {countIntersection(
          internBetalingsSoorten,
          rekeningGroepPerBetalingsSoort.map((rgpb) => rgpb.betalingsSoort),
        ) > 0 && (
          <Box>
            <Button
              color="success"
              variant={
                selectedCategorie === 'INTERN' ? 'contained' : 'outlined'
              }
              onClick={() => handleCategorieClick('INTERN')}
            >
              <InternIcon
                color={selectedCategorie === 'INTERN' ? 'white' : 'success'}
              />
            </Button>
            <Typography
              fontSize={11}
        ...
        <Box>
          <Button
            color="success"
            variant={
              selectedCategorie === 'UITGAVEN' ? 'contained' : 'outlined'
            }
            onClick={() => handleCategorieClick('UITGAVEN')}
          >
            <UitgavenIcon
              color={selectedCategorie === 'UITGAVEN' ? 'white' : 'success'}
            />
          </Button>
          <Typography
            fontSize={11}
      ...
      {selectedCategorie === 'UITGAVEN' &&
        renderBetaalSelectBlok({
          filterFn: (rgpb) =>
            uitgavenBetalingsSoorten.includes(rgpb.betalingsSoort),
        })}
        ...
        <>
          <Box mt={2}>
            ...
            >
              {rekeningGroepPerBetalingsSoort
                .filter((rgpb) =>
                  internBetalingsSoorten.includes(rgpb.betalingsSoort),
                )
                .map((rgpb, index) => (
                  <Button
                    ...
                    sx={{ mb: '13px' }}
                    variant={
                      selectedBetalingsSoort === rgpb.betalingsSoort
                        ? 'contained'
                        : 'outlined'
                    }
                    onClick={() =>
                      handleBetalingsSoortClick(rgpb.betalingsSoort)
                    }
                  >
            ...
            <Grid>
              {rekeningGroepPerBetalingsSoort
                ...
                .filter(
                  (rgpb) => rgpb.betalingsSoort === selectedBetalingsSoort,
                )
                .map((rgpb, rgpbIndex) => (
                  <Fragment key={rgpb.betalingsSoort + '-' + rgpbIndex}>
                    <Grid
                          ...
                          (rg) =>
                            ...
                            rg.rekeningen.map((rekening) => (
                              <Button
                                ...
                                onClick={() =>
                                  handleRekeningClick(rekening.naam)
                                }
                              >
                                {rekening.naam} <br />
                                {(stand &&
