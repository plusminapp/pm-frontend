...
export default function WijzigPeriodeDialoog(props: WijzigPeriodeDialoogProps) {
  const { actieveHulpvrager, setSnackbarMessage, stand } = useCustomContext();
  ...
  useEffect(() => {
    if (stand) {
      setFormSaldi(
        (stand.resultaatOpDatum as SaldoDTO[])
  ...
  const saveWijzigingen = async () => {
    if (actieveHulpvrager && props.periodes) {
      const saldos = formSaldi.map(
        (saldo) =>
          ({
    ...
    <React.Fragment>
      ...
      >
        <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
          ...
          eriodeopening op{' '}
          {dayjs(props.periodes[props.index].periodeStartDatum).format(
            'D MMMM',
          )}
        </DialogTitle>
        <IconButton
              ...
              <Table sx={{ width: '100%' }} aria-label="simple table">
                <TableBody>
                  <TableRow sx={{ '& td, & th': { border: 0 } }}>
                    ...
                    >
                      {dayjs(
                        props.periodes[props.index].periodeStartDatum,
                      ).format('D MMMM')}
                    </TableCell>
                    {props.editMode && (
                      <TableCell
