...
export default function Resultaat(props: PeriodeProps) {
  ...
  const calculateResult = (): number => {
    ...
    return saldoLijst.reduce((acc, saldo) => (acc + saldo.openingsSaldo), 0)
  }
          ...
          <TableBody>
            ...
            .map((saldo, index) => (
              <TableRow key={saldo.rekeningGroepNaam+index}>
                ...
                <TableCell align="right" size='small' sx={{ p: "6px" }}>{currencyFormatter.format(saldo.openingsSaldo)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
